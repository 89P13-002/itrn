...
class Neo4jVectorDB:
    ...
    def _create_indexes(self):
        ...
        with self.driver.session() as session:
            try:
                ...
                session.run("CREATE INDEX cluster_topic IF NOT EXISTS FOR (c:Cluster) ON (c.topic)")
                session.run("CREATE INDEX cluster_category IF NOT EXISTS FOR (c:Cluster) ON (c.category)")
                session.run("CREATE INDEX document_chunk_id IF NOT EXISTS FOR (d:Document) ON (d.chunk_id)")
    ...
    def create_or_get_cluster(self, cluster_metadata: Dict[str, Any], metadata_extractor: MetadataExtractor) -> str:
        ...
        cluster_id = f"{cluster_metadata['category']}_{cluster_metadata['domain']}_{cluster_metadata['topic']}"
    ...
    def find_similar_clusters(self, document_metadata: Dict[str, Any], metadata_extractor: MetadataExtractor, 
        ...
        with self.driver.session() as session:
            ...
            result = session.run("""
                CALL db.index.vector.queryNodes('cluster_metadata_embeddings', $max_clusters, $query_embedding)
                YIELD node, score
                WHERE score >= $similarity_threshold
                RETURN node.id as cluster_id, score
                ORDER BY score DESC
            """, 
            max_clusters=max_clusters * 2,  # Get more candidates to filter by threshold
            query_embedding=doc_metadata_embedding,
            similarity_threshold=similarity_threshold
            )
            
            similar_clusters = []
            for record in result:
                if len(similar_clusters) < max_clusters:
                    similar_clusters.append((record['cluster_id'], record['score']))
        ...
        with self.driver.session() as session:
            for i, cluster_id in enumerate(cluster_ids):
                ...
                session.run("""
                    ...
                    CREATE (d)-[:BELONGS_TO {similarity_score: $similarity_score}]->(c)
    ...
    def find_relevant_clusters_by_embedding(self, query_metadata: Dict[str, Any], metadata_extractor: MetadataExtractor, 
        ...
        with self.driver.session() as session:
            result = session.run("""
                CALL db.index.vector.queryNodes('cluster_metadata_embeddings', $top_k, $query_embedding)
                YIELD node, score
                RETURN node.id as cluster_id, node.topic, node.category, node.domain, 
                       node.keywords, node.tags, node.document_count, node.metadata_text, score
                ORDER BY score DESC
            """, top_k=top_k, query_embedding=query_metadata_embedding)
            
            clusters = []
            for record in result:
                clusters.append({
                    'cluster_id': record['cluster_id'],
                    'topic': record['node.topic'],
                    'category': record['node.category'],
                    'domain': record['node.domain'],
                    'keywords': record['node.keywords'],
                    'tags': record['node.tags'],
                    'document_count': record['node.document_count'],
                    'metadata_text': record['node.metadata_text'],
    ...
    def search_within_cluster(self, cluster_id: str, query_embedding: List[float], top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        with self.driver.session() as session:
            result = session.run("""
                ...
                CALL db.index.vector.queryNodes('document_content_embeddings', $top_k, $query_embedding)
                YIELD node, score
                WHERE node = d
                RETURN d.chunk_id as chunk_id, d.content as content, 
                       d.topic, d.category, d.domain, d.summary, d.tags,
                       r.similarity_score as cluster_similarity, score as content_similarity
                ORDER BY score DESC
            """, cluster_id=cluster_id, top_k=top_k, query_embedding=query_embedding)
    ...
    def get_cluster_stats(self) -> Dict[str, Any]:
        ...
        with self.driver.session() as session:
            ...
            return {
                ...
                'unique_document_chunks': len(set(c['cluster_id'].split('_')[0] for c in clusters)),
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Total Document Assignments: {stats['total_documents']}")
        print(f"Unique Document Chunks: {stats['unique_document_chunks']}")
        print(f"Multi-cluster Assignment Distribution: {stats['multi_cluster_assignment_stats']}")
        print(f"Hyperparameters: {stats['hyperparameters']}")
        
        for cluster in stats['clusters'][:5]:  # Show top 5 clusters
            print(f"- {cluster['topic']} ({cluster['category']}/{cluster['domain']}): {cluster['document_count']} docs")
        
        # Update hyperparameters if needed
        # rag.update_hyperparameters(max_clusters_per_document=3, cluster_similarity_threshold=0.8)
        ...
        print(f"\nQuery metadata: {result['query_metadata']}")
        print(f"\nRelevant clusters found (by metadata embedding similarity):")
        for cluster in result['relevant_clusters']:
            print(f"- {cluster['topic']} (similarity: {cluster['similarity_score']:.3f})")
        
        print(f"\nRetrieval Statistics: {result['retrieval_stats']}")
