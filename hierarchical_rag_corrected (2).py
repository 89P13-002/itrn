...
def build_index_from_documents():
    """Build and save the hierarchical index from documents"""
...
def load_existing_index():
    """Load pre-built hierarchical index"""
    faiss_index = HierarchicalFAISSIndex(config.EMBEDDING_DIM)
...
def process_queries_only(faiss_index):
    """Process queries using existing index"""
    ...
    logger.info("Processing queries with existing index...")
...
def check_index_exists():
    ...
    index_files = [
        os.path.join(config.INDEX_PATH, "coarse_index.faiss"),
        os.path.join(config.INDEX_PATH, "hierarchical_metadata.pkl")
    ]
    return all(os.path.exists(f) for f in index_files)


def main():
    """Main function with index reuse logic"""
    ...
    if check_index_exists():
        ...
        if faiss_index is not None:
            # Use existing index for queries
            process_queries_only(faiss_index)
        else:
